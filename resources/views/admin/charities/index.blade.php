...
@section('page_title','قائمة الجمعيات');
        ...
        <table class="table table-hover table-bordered text-center " id="datatable">
            ...
            @foreach($charities as $charity)
                <tr class="">
                    ...
                    <td>{{$charity->name}}</td>
                    <td>
                        <a href="{{route('charity.edit',$charity->id)}}" class="btn btn-primary btn-sm">تعديل</a>
                    </td>

                    <td>
                        <form action="{{route('charity.destroy',$charity->id)}}" method="post" id="delete">
                            @csrf
                            @method('delete')
