            ...
            <div class="row">
                <div class="col-3 users-list p-0 border">
                    ...
                    @foreach($chat as $user)
                        <div class=" border p-1 user text-right  {{(isset($chat_id)&&$chat_id==$user->id)?"bg-primary":""}}">
                            <a class="{{(isset($chat_id)&&$chat_id==$user->id)?"text-white":""}}" href="{{route('getMessages',$user->id)}}">
                          ...
                          @if($user->user_id == auth()->id())
                                        {{$user->user2->name}}
                                    @else
{{--                              @dd([$user->user_id,$user->user_id2,$user->user1_id])--}}
                                        {{$user->user1->name}}
                ...
                <div class="col-8 p-0 border">
                    ...
                    <form action="">
                        <div class="form-group justify-content-center message-form ">
                            ...
                            <textarea name="message_content" id="message-text"
                                      class="form-control mr-4 message-text"></textarea>
